...
def process_file(file_path: str, filename: str, source_type: str):
    """Parse -> Chunk -> sace locally -> embed -> index in Qdrant."""
    with logfire.span("Processing File", file=filename, source=source_type):
        try:
            # 1. Extranction of text based on file extension
...
def run_universal_ingestion(base_dir: str, explicit_source_type: str = None, wipe: bool = False):
    """Scan base_dir, map sub-folders to source types, and ingest all doucments.
    ...
    with logfire.span("Universal Ingestion Started", base_directory = base_dir):
        if wipe and qdrant_client.collection_exists(settings.QDRANT_COLLECTION):
            logfire.info(f"Wiping collection '{settings.QDRANT_COLLECTION}' as requested.")
        ...
        if not subdirs:
            if explicit_source_type:
                ...
                source_type = (
                    ...
                    else "general" if "general" in base_name
                    else None
                )
        
            logfire.info(f"Using source_type = '{source_type}'")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(target_dir):
        print(f"Error: path '{target_dir}' does not esists.")
        sys.exit(1)
